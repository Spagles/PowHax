...
public class Config {
    ...
    public static Path writeNewShopData(Map<String, PriceScraper.ItemForSale> itemsForSale) {
        Path file = nextAvailableFile(LocalDate.now(), Utils.getWorldName());
    ...
    private static Path nextAvailableFile(LocalDate date, String worldName) {
        ...
        Path file;
        do {
            file = SHOP_FOLDER.resolve("%s_%s_%d.json".formatted(datePrefix, safeWorldName, index++));
